...
export default function TrendContext({ trend }: { trend: Trend }) {
  ...
  return (
    <>
      {tweets.length > 0 && (
        <section className="my-5 rounded-2xl border border-gray-200 bg-white p-5 dark:border-white/10 dark:bg-night-card">
          <h2 className="mb-3 flex items-center gap-2 text-base font-bold text-ink dark:text-white">
      ...
      {hasOtt && (
        <section className="my-5 rounded-2xl border border-gray-200 bg-white p-5 dark:border-white/10 dark:bg-night-card">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2 text-sm">
            {ott?.kind && (
              <span className="rounded-full bg-gray-100 px-2.5 py-0.5 font-medium text-gray-700 dark:bg-white/10 dark:text-gray-300">
                {ott.kind}
              </span>
            )}
          ...
          {ott?.synopsis && (
            <p className="mt-3 text-[15px] leading-relaxed text-gray-800 dark:text-gray-200">
              {ott.synopsis}
            </p>
          )}
        </section>
              ...
              <li key={`${n.url}-${i}`} className="flex gap-3">
                ...
                <div className="min-w-0">
                  <a
                    ...
                    className="text-[15px] font-medium leading-snug text-gray-800 hover:text-brand dark:text-gray-200"
                  >
                    {n.title}
                  </a>
                  {n.source && (
        ...
        <section className="my-5 rounded-2xl border border-gray-200 bg-white p-5 dark:border-white/10 dark:bg-night-card">
          ...
          <ul className="mt-3 space-y-3">
            {comments.map((c, i) => (
              <li
                key={i}
                className="rounded-xl bg-gray-50 p-3 text-sm text-gray-800 dark:bg-white/5 dark:text-gray-200"
              >
